  ...
  if (!stocks || stocks.length === 0) {
    return (
      <div className="hidden md:block bg-slate-800/80 border border-slate-700/60 rounded-xl p-12 text-center text-slate-400">
        ...
        <p className="text-sm text-slate-500 mt-1">상단의 "종목 추가" 또는 "CSV 업로드"를 통해 포트폴리오를 구성해보세요.</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900/60 border-b border-slate-700 text-xs font-semibold text-slate-400 tracking-wider">
              ...
              <th className="py-3.5 px-4 text-right">현재가 (구글 파이낸스)</th>
              <th className="py-3.5 px-4 text-right">보유수량</th>
              <th className="py-3.5 px-4 text-right">매수총액(원화)</th>
              <th className="py-3.5 px-4 text-right">평가총액(원화)</th>
              <th className="py-3.5 px-4 text-right">손익금액(원화)</th>
            ...
            {stocks.map((stock) => {
              ...
              return (
                ...
                >
                  ...
                  {/* 매수가 (미국 주식은 달러+원화 환산 병기) */}
                  <td className="py-4 px-4 text-right">
                    {stock.isUs ? (
                      <div>
                        <div className="text-white font-medium">{formatUsd(stock.buyPrice)}</div>
                        <div className="text-[11px] text-slate-400 font-normal">₩{formatNumber(stock.buyPriceKrw)}</div>
                      </div>
                    ) : (
                      <div className="font-medium text-slate-300">{formatNumber(stock.buyPrice)}원</div>
                    )}
                  </td>
                  ...
                  <td className="py-4 px-4 text-right font-semibold">
                    {stock.isUs ? (
                      <div>
                        <div className="text-white font-bold">{formatUsd(stock.currentPrice)}</div>
                        <div className="text-[11px] text-slate-400 font-normal">₩{formatNumber(stock.currentPriceKrw)}</div>
                      </div>
                    ) : (
                      <div className="text-white">{formatNumber(stock.currentPrice)}원</div>
                    )}
                    {stock.changePrice !== undefined && stock.changePrice !== 0 && (
                      <div className={`text-xs flex items-center justify-end gap-0.5 mt-0.5 ${getStockColorClass(stock.changeRate)}`}>
                        ...
                        <span>{stock.changePrice > 0 ? '+' : ''}{stock.isUs ? formatUsd(stock.changePrice) : formatNumber(stock.changePrice) + '원'} ({stock.changeRate > 0 ? '+' : ''}{stock.changeRate}%)</span>
                      </div>
                    )}
                  </td>
